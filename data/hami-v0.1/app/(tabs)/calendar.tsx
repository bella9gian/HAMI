...
import { CalendarEvent, createCalendarEvent, deleteCalendarEvent, deleteOccurrence, FamilyMember, formatEventTime, loadEventDays, loadEventsForDate, loadEventsForRange, Recurrence, RECURRENCE_OPTIONS, toDateKey, updateCalendarEvent } from '@/lib/calendar';
...
export default function Calendar() {
  ...
  function openEdit(event: CalendarEvent) {
    // Remember which day was tapped, for "delete this occurrence".
    setOccurrenceKey(toDateKey(new Date(event.startsAt)));
  ...
  async function removeOccurrence() { if (!editingEvent || !occurrenceKey) return; setSaving(true); setError(''); try { await deleteOccurrence(editingEvent.id, occurrenceKey); setEvents(await loadEventsForDate(selectedDate)); await refreshEventDays(); await refreshRange(); setShowForm(false); resetForm(); } catch (err: any) { setError(err?.message ?? 'Unable to delete this event.'); } finally { setSaving(false); } }
    ...
    {showForm && <Card style={s.form}>
      ...
      {editingEvent && (editingEvent.recurrence && editingEvent.recurrence !== 'none' ? (
        <View style={s.deleteRow}>
          <Pressable onPress={removeOccurrence} disabled={saving} style={[s.deleteHalf, s.deleteHalfBorder]}><Text style={s.deleteText}>Delete this day</Text></Pressable>
          <Pressable onPress={removeEvent} disabled={saving} style={s.deleteHalf}><Text style={s.deleteText}>{confirmDelete ? 'Tap again: whole series' : 'Delete series'}</Text></Pressable>
        </View>
      ) : (
        <Pressable onPress={removeEvent} disabled={saving} style={s.deleteButton}><Text style={s.deleteText}>{confirmDelete ? 'Tap again to delete permanently' : 'Delete event'}</Text></Pressable>
      ))}
    </Card>}
