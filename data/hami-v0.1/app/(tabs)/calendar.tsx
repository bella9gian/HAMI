...
const WEEK_SPAN = 26; // weeks navigable either side of today
...
export default function Calendar() {
  ...
  const monthLabel = monthYearFormatter.format(weeks[centerWeek]?.[3]?.day ?? dateFromKey(todayKey));

  useEffect(() => {
    if (stripWidth && !didInitStrip.current) {
      didInitStrip.current = true;
      requestAnimationFrame(() => stripRef.current?.scrollTo({ x: WEEK_SPAN * stripWidth, animated: false }));
    }
  }, [stripWidth]);
...
const s = StyleSheet.create({
  calStrip: { paddingTop: 4 }, calHead: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 4, paddingBottom: 4 }, monthLabel: { fontSize: 15, fontWeight: '800', color: colors.text },
  days: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10 }, day: { alignItems: 'center', gap: 7, flex: 1 }, dayName: { fontSize: 11, color: colors.muted, fontWeight: '700' }, dayNum: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' }, active: { backgroundColor: colors.forest }, num: { fontWeight: '700', color: colors.text }, activeText: { color: '#fff' }, todayRing: { borderWidth: 1.5, borderColor: colors.forest }, todayText: { color: colors.forest },
